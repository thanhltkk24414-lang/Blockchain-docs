...
4. Copy hai giá trị từ console (hoặc dùng **Cách 1** — nút **Copy JSON cho Postman**):
   - Toàn bộ **message** (nhiều dòng) → Postman: `siweMessage` (collection dùng pre-request script) · REST Client: request **03b** + `verify-body.json`
   - **signature** (chuỗi hex `0x...`) → Postman: `siweSignature` · REST Client: trong cùng file JSON
...
1. Đảm bảo `siweMessage` và `siweSignature` đã điền trong environment **hoặc** body chứa JSON từ **Copy JSON cho Postman**
2. **Postman:** import lại `backend/postman/Fapex.postman_collection.json` nếu bản cũ chưa có pre-request script
3. **POST /api/auth/verify** → **Send**

Collection **tự build body** bằng pre-request script (`JSON.stringify`) — bạn có thể dán message SIWE nhiều dòng vào biến `siweMessage` mà không bị lỗi JSON.

Body mẫu (Postman — chỉ minh họa; script ghi đè khi Send):
...
**Cách nhanh không cần biến:** dán nguyên JSON từ siwe-sign.html vào tab Body → raw → JSON.
